...
export default new Event({
  event: "messageUpdate",
  execute: async (oldMessage: Message, newMessage: Message, client: Client) => {
    // Ignore direct messages or bots, or if content didn't change
    if (!newMessage.guild || newMessage.author?.bot) return;
    if (oldMessage.content === newMessage.content) return;

    Logger.info(`Message edited in channel #${(newMessage.channel as any).name} (Guild: ${newMessage.guild.name})`);
