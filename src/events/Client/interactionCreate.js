...
export default new Event({
  ...
  execute: async (interaction, client) => {
    ...
    const getComponentHandler = (collection, customId) => {
      // 1. Exact match
      if (collection.has(customId)) {
        return { handler: collection.get(customId), params: [] };
      }
        ...
        if (subCommandName) {
          ...
          if (subCommandHandler) {
            try {
              ...
              const errMsg = { content: config.messages.INTERACTION_ERROR, ephemeral: true };
    ...
    if (interaction.isButton()) {
      ...
      if (handler) {
        ...
        try {
          ...
          return await interaction.reply({ content: config.messages.INTERACTION_ERROR, ephemeral: true }).catch(() => {});
        }
      }
    }
