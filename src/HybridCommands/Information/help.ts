...
export default new HybridCommand({
  ...
  execute: async (ctx: CommandContext, client: Client) => {
    ...
    client.prefixCommands.forEach((cmd: any) => {
      ...
      if (isSub) {
        ...
        if (subOptions.length > 0) {
          const list = subOptions
            .map((opt: any) => `🔹 **${opt.name}** (${optionTypeNames[opt.type] || "Unknown"})${opt.required ? ' *' : ''}: *${opt.description}*`)
            .join("\n");
        ...
        if (targetCmd.aliases && targetCmd.aliases.length > 0) {
          embed.fields.push({ name: "Aliases", value: targetCmd.aliases.map((a: string) => `\`${prefix}${a}\``).join(", "), inline: true });
        }
        ...
        if (targetCmd.permissions?.user && targetCmd.permissions.user.length > 0) {
          embed.fields.push({
            ...
            value: targetCmd.permissions.user.map((p: string) => `\`${p}\``).join(", "),
            inline: true,
          });
        }
        ...
        if (targetCmd.examples && targetCmd.examples.length > 0) {
          ...
          const exList = targetCmd.examples.map((ex: string) => `\`${currentPrefix}${ex}\``).join("\n");
